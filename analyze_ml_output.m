...
netoutPrint = true
resUnderSwitch = false

if netoutPrint == true
  figure(1),clf,hist(targets(3,abs(targets(3,:))<1.7),50);
  saveas(figure(1),['ml_analyze/simple_para3_target_test/targets_figure_3_under_1.7.fig']);

  % print targets
  for i=1:5
    figure(i),clf,hist(targets(i,:),50);
    saveas(figure(i),['ml_analyze/simple_para3_target_test/targets_figure_' num2str(i) '.fig']);
  end

  % print netout
  for i=1:5
    figure(i),clf,hist(netout(i,:),50);
    saveas(figure(i),['ml_analyze/simple_para3_target_test/netout_figure_' num2str(i) '.fig']);
  end
end


if resUnderSwitch == true
  count_res=[0,0,0,0,0];
  pos_res2=[];
  resUnder2=[];
  pos_res1=[];
